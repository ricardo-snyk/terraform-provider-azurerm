...
func resourceArmMsSqlTransparentDataEncryption() *schema.Resource {
...
func resourceMsSqlTransparentDataEncryptionRead(d *schema.ResourceData, meta interface{}) error {
	...
	serverKeyType := ""
	if resp.EncryptionProtectorProperties != nil && resp.EncryptionProtectorProperties.ServerKeyType != "" {
		serverKeyType = string(resp.EncryptionProtectorProperties.ServerKeyType)
	}
	if err := d.Set("server_key_type", serverKeyType); err != nil {
		return fmt.Errorf("setting server_key_type`: %+v", err)
	}
